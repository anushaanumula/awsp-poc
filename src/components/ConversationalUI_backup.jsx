...
const ConversationalUI = ({ context, onSelectSite, onClose }) => {
  ...
  const generateResponse = (userInput) => {
    ...
    if (input.includes('status of') || input.includes('check site')) {
      const siteMatch = input.match(/[a-z]{3}\d{3}/i);
      if (siteMatch) {
        return {
          type: 'site_status',
          data: {
            site: siteMatch[0].toUpperCase(),
            status: 'Operational',
            uptime: '99.8%',
    ...
    if (input.includes('optimize') || input.includes('improve')) {
      ...
      return {
        type: 'optimization',
        data: {
          site: siteMatch ? siteMatch[0].toUpperCase() : 'Selected Site',
          recommendations: [
            'Adjust antenna tilt by +3 degrees',
            'Increase power allocation to Band 41',
            'Enable carrier aggregation',
            'Schedule maintenance window for hardware upgrade'
          ]
        }
      };
    }
